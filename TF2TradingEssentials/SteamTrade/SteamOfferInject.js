setTimeout(function() {
    $$("#mainContent")[0].insert({before:"<div><center><h1 style='margin-top:20px;font-size:30px;'>Team Fortress 2 Trading Essentials</h1></center>"+
    ...
    $$("#mainContent")[0].insert({after:"<div><center><h4 style='margin-top:20px;font-size:15px;'>Team Fortress Trading Essentials Copyrighted By: Absolute Stratos</h4></center>"+
    ...
    function theAddLooper (a,i,amount){
        //Get all the items
        var items = jQuery("div[id*= 'inventory_'][id*='_440_2'][style!='display: none;']:last>.inventory_page").find('.itemHolder');
        //a is number of items added, i is the current item in the entire list, amount is the requested amount
        if(i < items.length && a < amount && runner){ 
            //checking to see if item is filtered or not
            if(jQuery(items[i]).css('visibility') != 'hidden' && jQuery(items[i]).css('display') != 'none'){ 
                ...
                if(items[i].getElementsByTagName('div')[0]) {
                    ...
                    if(items[i].getElementsByTagName('div')[0].id) {
                        //get the id of the itd we are going to add
                        var orgid = items[i].getElementsByTagName('div')[0].id;
                        addItem(items[i]);
                        //Seeing if its their inventorys that selected
                        var theirs = false;
                        if(jQuery('#inventory_select_their_inventory').attr('class').indexOf('active')!=-1){
                            theirs = true;
                        }
                        //Wait for the item to move to the inventory slot
                        theAddWaiter(a,i,amount,orgid,theirs);
                    }else{
                        i+=1;
    ...
    function theClearLooper (theirs){
        if(theirs){
            var items =jQuery("#their_slots>div[id*='their_slot_']>.slot_inner").find("div[id*='item440_2_']");
        }else{
        ...
        if(items.length > 0 && runner){
            ...
            setTimeout(function(){theClearWaiter(orgid,theirs);},100);
        }
    }
    
    function theAddWaiter (a,i,amount,orgid,theirs){
        //Checking whos trade to look at
        if(theirs === true){
            var newid=jQuery("#their_slots>div[id*='their_slot_']>.slot_inner").find('#'+orgid);
        }else{
            var newid=jQuery("#your_slots>div[id*='your_slot_']>.slot_inner").find('#'+orgid);
        }
        //Checking to see if the item was added
        if(newid[0]){
            if(orgid==newid[0].id) {
                a+=1;
                i+=1;
        ...
        //Wait some more
        setTimeout(function (){theAddWaiter(a,i,amount,orgid,theirs);},5);
    }
    
    function theClearWaiter (orgid,theirs){
        if(theirs){
            var newid =jQuery("#their_slots>div[id*='their_slot_']>.slot_inner").find("#"+orgid);
        }else{
            var newid =jQuery("#your_slots>div[id*='your_slot_']>.slot_inner").find("#"+orgid);
    ...
    if(g_ulTradePartnerSteamID){
        ...
        .done(function(data) { 
            if(data){
                ...
                jQuery('#steamRep').html("<table><tr><td style='width:"+jQuery('#steamRep').width()+"px'><span style='float:left'><h3 style='float:top;font-size:20px;margin-left:20px; margin-top:10px;margin-bottom:5px'>You're currently making an offer with:</h3>"+
                                         ...
                                         "<span style = 'float:right'><a href='http://backpack.tf/profiles/"+steam64+"' target='_blank'><img style='border:0; margin-top:10px; margin-right:10px' src='http://backpack.tf/images/logo_440.svg' alt='BP.tf' height='18px'></a></span>"+
...
setTimeout(function(){
    ...
    var theirItems = null;
    var yourItems = null;
    jQuery("body").on('mouseenter',".itemHolder>div[id*='item440_2_']", function() { hoverEleme = this; othersItems = false;});
    jQuery("#your_slots>div[id*='your_slot_']>.slot_inner").on('mouseenter',"div[id*='item440_2_']", function() { hoverEleme = this; othersItems = false;});
    jQuery("#their_slots>div[id*='their_slot_']>.slot_inner").on('mouseenter',"div[id*='item440_2_']", function() { hoverEleme = this; othersItems = true;});
    if(typeof(Storage)!=="undefined"){
        ...
        jQuery.ajax({
            url: "https://api.steampowered.com/IEconItems_440/GetPlayerItems/v0001/?SteamID="+g_steamID+"&format=json&key=9E35E15F84D8483CB0502D0B68ECA324",
            cache: false,
            dataType: 'json',
        ...
        jQuery.ajax({
            url: "https://api.steampowered.com/IEconItems_440/GetPlayerItems/v0001/?SteamID="+g_ulTradePartnerSteamID+"&format=json&key=9E35E15F84D8483CB0502D0B68ECA324",
            cache: false,
            dataType: 'json',
            success: function( data ){
                theirItems = data;
        ...
        jQuery('#mainContent').on('DOMNodeInserted', function(e) {
            ...
            if (jQuery(e.target).is('#hover')) {
                ...
                if(localStorage.getItem('backpackPriceCache')){
                    ...
                    $$("#hover>.item_desc_description")[0].insert( {after:"<div id='itemPriceBox' style='margin-bottom:10px;margin-right:20px;border-style:solid;border-width:3px;border-color:#5E5E5E;'>"+
